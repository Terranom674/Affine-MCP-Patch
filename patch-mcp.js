...
for (const marker of ['private readonly models: Models', 'async deleteDoc(_workspaceId: string, _docId: string)', 'return;'])
...
for (const marker of [
  ...
  'Deleted workspace ${workspaceId} doc ${docId}'
]) if (!docModelSource.includes(marker)) fail(`DocModel delete routine changed; missing marker: ${marker}`);
...
const deleteTool = `{
name:\"delete_document\",title:\"Delete Document\",description:\"Permanently delete a document using AFFiNE stable removeDoc semantics and AFFiNE's persisted DocModel delete routine. This cannot be undone.\",
...
for (const marker of ['create_document','update_document','update_document_meta','delete_document','Doc.Delete','file:///opt/affine-mcp-patch/node_modules/yjs/dist/yjs.mjs','meta.pages','removeDoc','DocModel delete routine is unavailable'])
  if (!patchedBundle.includes(marker)) fail(`Required marker disappeared: ${marker}`);

fs.writeFileSync(bundlePath, patchedBundle);
console.log('[AFFiNE MCP Patch] Enabled MCP WRITE gates and added delete_document using AFFiNE stable removeDoc semantics plus AFFiNE DocModel persisted-data cleanup.');
